...
def pipeline_runs() -> list[dict]:
    return fetch_all("""
        SELECT run_id::text, started_at, finished_at, status, source_file_count,
...
def metrics(limit: int = 100) -> list[dict]:
    return fetch_all("""
        ...
        FROM gold.annual_metric_summary
...
def metric_lineage(metric_name: str, fiscal_year: int | None = None) -> list[dict]:
    return fetch_all("""
        ...
        WHERE metric_name ILIKE :metric_name
